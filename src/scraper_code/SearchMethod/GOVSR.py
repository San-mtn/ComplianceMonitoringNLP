...
class GOVSR(SearchMethod):
    ...
    def search(self, **kwargs):
        """
            Searches on gov.sr for the link of the organisation
            gov.sr contains all government organisations, and some have website links on it
            :param **kwargs:
        ...
        for page_nr in range(6):
            url = f'https://gov.sr/ministeries/ministerie-van-financien-en-planning/jaarrekeningen-staatsbedrijven-en-instanties/{page_nr}'
            ...
            govsrNames += [span.text for span in soup.find_all("span", "elementor-heading-title elementor-size-default")]
            govsrURLS += [span['href'] for span in soup.find_all("a", "elementor-button-link elementor-button elementor-size-xs")]

        return govsrNames, govsrURLS
